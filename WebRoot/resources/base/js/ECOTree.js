ECONode = function (id, pid, dsc, t, w, h, c, bc, target, meta) {
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
	this.w = w;
...
ECOTree = function (obj, elm) {
	this.config = {
		...
		nodeSelColor : "#6A5ACD",
		//levelColors : ["#CCCCFF","#CCCCFF","#CCCCFF","#CCCCFF"],
		//levelBorderColors : ["#CCCCFF","#CCCCFF","#CCCCFF","#CCCCFF"],
		colorStyle : ECOTree.CS_LEVEL,
		useTarget : true,
		...
		expandedImage : '/gcrm/resources/img/less.gif',
		collapsedImage : '/gcrm/resources/img/plus.gif'
	}
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				...
				case "VML":
					...
					if (this.config.selectMode != ECOTree.SL_NONE)
						s.push('href="javascript:'+this.obj+'.selectNode(\''+node.id+'\', true,\'' + node.t + '\');" ');										
					s.push('>');
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<canvas id="ECOTreecanvas" width=800 height=400></canvas>');
			break;
		...
		case "VML":
			s.push('<v:group coordsize="800, 400" coordorigin="0, 0" style="width=800px;height=400px;" >');
			s.push(this._drawTree());
			s.push('</v:group>');
			break;
	}
...
ECOTree.prototype.add = function (id, pid, dsc, t, w, h, c, bc, target, meta) {	
	...
	var node = new ECONode(id, pid, dsc, ty, nw, nh, color, border, tg, metadata);	//New node creation...
...
ECOTree.prototype.selectNode = function (nodeid, upd, ty) {		
