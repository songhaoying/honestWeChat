...
<script type="text/javascript" charset="UTF-8">
	...
	$(function(){
		$("#COMPANY_NAME").focus(); 
		$.post("<%=basePath%>/backstage/loveorganajax_getLoveOrganOne.action",{'loveOrgan_Info.COMPANY_ID':'<%=id%>'},function(data){
			$("#COMPANY_DESC").val(data[0].COMPANY_DESC);
			loadckeditor();
			$("#COMPANY_NAME").val(data[0].COMPANY_NAME);
			$("#COMPANY_ADDRESS").val(data[0].COMPANY_ADDRESS);
			var SERVICE_CONTENT = data[0].SERVICE_CONTENT;
			$("#SERVICE_CONTENT").val(SERVICE_CONTENT);
			if(SERVICE_CONTENT.indexOf("：") > -1){
				SERVICE_CONTENT = SERVICE_CONTENT.replace(/：/g, ",");
			}
			if(SERVICE_CONTENT.indexOf("；") > -1){
				SERVICE_CONTENT = SERVICE_CONTENT.replace(/；/g, ",");
			}
			if(SERVICE_CONTENT.indexOf(",") > -1){
				var SERVICE = SERVICE_CONTENT.split(",");
				for(var i = 0; i < SERVICE.length; i++){
					var treeObj=$.fn.zTree.getZTreeObj("serviceTree");
					var node = treeObj.getNodeByParam("SERVICE_TYPE_NAME", SERVICE[i]);
					if(node.SERVICE_TYPE_PARENT_ID != null && node.SERVICE_TYPE_PARENT_ID != '0'){
						treeObj.checkNode(node,true,true);
					}
				}
			}
			$("#COMPANY_LOGO").val(data[0].COMPANY_LOGO);
			$("#image").attr("src", "<%=basePath%>"+data[0].COMPANY_LOGO);
			$("#ORD_ID").val(data[0].ORD_ID);
			var SERVICE_LEVEL = data[0].SERVICE_LEVEL;
			$("#SERVICE_LEVEL .xing").each(function(){
				var dic = $(this).attr("DIC")
				if(dic == SERVICE_LEVEL){
...
<body>
	<div class="tanchuceng" style="background-image: url('../images/dialog_title1.png');">
		<div class="damo" style="margin-left: 400px;">修改义工活动信息</div>
	...
	<div class="center" style="height: 440px; float: left; padding-left: 5px;">
		<table id="mytable" cellspacing="1">
			...
			<tr>
				...
				<td colspan="2" style="text-align: center;"><input type="hidden" id="COMPANY_LOGO" />
			...
			<tr style="border: 0px;">
				...
				<td style="border: 0px; height: 25px;"><input onclick="return save();" type="button" style="height: 25px; float: right; background-color: #CAE8EA; width: 80px;" value="  修  改  "></td>
